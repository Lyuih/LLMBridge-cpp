...
// 可接入任意 OpenAI 兼容端点(gpt / qwen / kimi 等),模型名通过配置的 model_name 指定
// 支持 Function Calling: 注入 tools,解析 tool_calls

#ifndef GPTPROVIDER_H
#define GPTPROVIDER_H

#include "LLMProvider.h"

namespace chat_sdk
{
    class GPTProvider : public LLMProvider
    {
    public:
        // 模型初始化
    ...
    private:
        std::string model_name_ = "chat-4o-mini"; // 默认模型,可通过配置覆盖
        std::string model_desc_ = "OpenAI 的通用对话模型,可通过配置指向任意 OpenAI 兼容端点";
    };
}
